...
def menu(screen, size_screen):
    ...
    #Show data of player 2
    #start_button = bt.Button('Show data',size_screen/2.3, size_screen/1.11, size_screen*0.1,size_screen*0.053,font_player,(77,106,149),'white')
    ...
    while running:
        ...
        for event in events:
            ...
            if data1_on_button.mouse_click(event) or data1_off_button.mouse_click(event):
                data1_on = not data1_on
            if data2_on_button.mouse_click(event) or data2_off_button.mouse_click(event):
        ...
        if data1_on:
            data1_on_button.draw(screen)
        else:
            data1_off_button.draw(screen)

        if data2_on:
            data2_on_button.draw(screen)
        else:
            data2_off_button.draw(screen)
